...
const getUsers = async (req, res, next) => {
	let users;
	try {
		users = await User.find({}, "-password"); //omitts password//
	} catch (err) {
...
const login = async (req, res, next) => {
	...
	let token;
	try {
		token = jwt.sign({ userId: existingUser.id, email: existingUser.email, token: token }, process.env.JWT_KEY, { expiresIn: "1h" });
	} catch (err) {
